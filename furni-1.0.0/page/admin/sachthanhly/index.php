                        ...
                        <div class="card-body table-full-width table-responsive">
                            <form id="thanhLyForm" method="post">
                                <table class="table table-hover table-striped">
                                    ...
                                    <tbody> <?php if (count($sachthanhly) > 0): ?>
                                            <?php foreach ($sachthanhly as $item): ?>
                                                <tr>
                                                    ...
                                                    <td><?= $item["tacGia"] ?></td>
                                                    <td> <button type="submit" name="btXoaSingle" value="<?= $item["maSach"] ?>"
                                                            class="btn btn-danger">Thanh Lý</button> </td>
                                                </tr> <?php endforeach; ?> <?php else: ?>
                                            <tr>
                                                <td colspan="5" class="text-center">Chưa có sách cần Thanh Lý</td>
                                            </tr> <?php endif; ?>
                                    </tbody>
                                </table> <?php if (count($sachthanhly) > 0): ?> <button type="submit"
                                        class="btn btn-danger ml-custom">Thanh Lý Sách Đã Chọn</button> <?php endif; ?>
                            </form>
                        </div>
                    </div>
                </div>
